...
1. `call_model`: Configuration 로드 → `ChatOpenAI.bind_tools(TOOLS)` → 시스템 프롬프트(`{system_time}` 포맷) + 메시지 → LLM 호출
2. `route_model_output`: `AIMessage.tool_calls` 존재 여부로 분기
3. `tools` (ToolNode): tool_calls 병렬 실행, `ToolMessage`로 반환
4. `is_last_step` 가드: 재귀 한계 도달 시 도구 호출 대신 사과 메시지 반환
